 ...
 *
 * `runway/src/sbxdiff/pages/tlsfp.html` asks `/__sbxdiff/headers` for the order
 * the server actually received, from both sides, for a GET and for a POST --
 * the same request to the same place, which is the only way the two are
 * comparable. Chromium's order is identical across runs.
 */
 ...
 *
 * Measured, with `runway/src/sbxdiff/pages/navorder.html`: it does. The three
