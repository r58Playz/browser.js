...
import { _Map as Map, _Set as Set } from "@/shared/snapshot";
...
export function initializeCsp(client: ScramjetClient): CspState {
	...
	const visit = (
		...
		apply(node);
		for (const child of children(node)) visit(child, children, apply);
	};
	...
	const consume = (records: MutationRecord[]) => {
		...
		for (let i = records.length - 1; i >= 0; i--) {
			...
			if (record.type === "attributes") {
				...
				for (const child of added) parents.set(child, null);
				const restored = list(record.removedNodes);
				for (const child of restored) parents.set(child, node);
				children.set(node, [
					...kids(node).filter((child) => !added.includes(child)),
					...restored,
				]);
			}
		}
