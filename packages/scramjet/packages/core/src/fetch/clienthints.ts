...
export function readyClientHints(): Promise<void> {
	if (values !== null) return Promise.resolve();
	...
	pending = uaData
		.getHighEntropyValues(Object.values(HIGH_ENTROPY).map((h) => h.key))
		.then((raw) => {
			const out: Record<string, string> = {};
			for (const [header, { key, kind }] of Object.entries(HIGH_ENTROPY)) {
