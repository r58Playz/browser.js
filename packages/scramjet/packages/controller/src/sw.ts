...
export async function route(event: FetchEvent): Promise<Response> {
	try {
		...
		// `content-length` on every POST that is not an explicit `duplex:
		// "half"` upload. Measured against Chromium 155, a navigation POST puts
		// it third, in the framing block:
		//
		//     Host, Connection, Content-Length, Cache-Control, sec-ch-ua, ...
		//
		// FINDINGS #185 left this as the one wire difference remaining and put
		// the fix in the transport, because `content-length` is a forbidden
		// header name in fetch and the shim can never read the one the page
		// sent. It does not have to: reading the body gives the same number,
		// and a transport given bytes rather than a stream frames the length
		// itself.
		//
