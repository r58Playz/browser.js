...
191.  **The same-origin boundary between two guest origins does not exist, and
      ...
      It is the PROXY ITSELF. A pure-forwarding proxy, denying nothing at all,
      hangs the sandbox just the same. Scramjet compares windows by identity --
      message routing, `wrapfn`, the box lookup -- and a Proxy is never `===`
      the window it wraps, so the comparisons stop matching and the run stalls.
      ...
      There is no cheap way to test whether that `eval` read is what 600010 is
      about. Denying it wholesale -- defining a throwing `eval` on the widget's
