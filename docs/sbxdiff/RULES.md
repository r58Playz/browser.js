...
191.  **The same-origin boundary between two guest origins does not exist, and
      ...
      same way HANGS the sandbox. The harness and the controller drive guest
      frames through that same accessor, and a locked-down window denies them
      the members they need. Closing it wants a way to tell the proxy's own
      reads from the page's -- the native accessor already exists for exactly
      that (`client.native[iface](el).contentWindow`), so the work is to route
      scramjet's internal reads through it.
