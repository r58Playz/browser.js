...
4. **Do not add CDP to the record/replay path.** `Runtime.addBinding` adds an enumerable
   ...
   arguments, which *invokes page getters*; `--remote-debugging-pipe` sets
...
5. **Compare literally.** Never run scramjet's un-rewrite functions over a value before
   comparing it. The un-rewritten value *is* the bug.
6. **`diffClass` classifies, it never normalizes.** It runs strictly *after* literal
...
9. **Every new interceptor goes through `client.Intercept` / `client.proxyObject`.**
   ...
   *and* invisible to the `missing-interceptor` heuristic — it never populated
...
10. **Guest-op brackets are token-returning, not depth-balanced.** Per-instance
    `RawTrap`s (e.g. `xmlhttprequest.ts:79-113`) are installed *inside* an apply
...
13. **Never let the oracle silently ignore an input.** FIVE instances now, the last
    one *in the switch added to prevent the other four*: `--sbxdiff-debug-disable` was
...
14. **A replay miss is a divergence, not an error.** Emit `net_replay_miss` at
    ...
    hermeticity) and never abort the process (you lose *why* the sandbox asked).
...
15. **Never trust a build's reported exit status; grep the log.** `autoninja ... | tail`
    ...
    `$?` is *also* not enough for a backgrounded build: the job's exit code is that of
...
16. **Wrapping an installed V8 callback pointer means updating the context snapshot
    reference table too.** `v8_context_snapshot_generator` serializes the *installed*
...
17. **Every determinism claim needs N identical runs, not two that agree.** The P4
    randomness gate was recorded as passing on a byte-identical *pair*; a 5-run loop
    ...
    it was 5/5 deterministic *per configuration*.
18. **When replacing a mechanism, the gate must run without the old one.** The
    ...
    exercised the configuration being moved *away from*. This is a control-set gap,
...
19. **When a bisect comes back negative on every arm, find the input you never
    ...
    *driver* (`--dump-dom`/CDP vs the in-binary runner), which was constant in every
...
20. **Determinism gates must run a multi-process page.** Every gate here used a
    ...
    why a keyed-PRNG bug that gave *every* renderer the same byte sequence (duplicate
...
22. **"Invisible to reflection" is not "no observable effect."** The tracer stored
    ...
    caches, which a page times on its *own* objects. **Never mutate a page object.**
...
24. **Anything true only of Chromium 155 goes in `PINNED_ASSUMPTIONS.md`** with a check
    command. Several of these fail *silently* on a roll.
...
26. **Code that compiles and is correct can still never run.** `MaybeCreateSbxdiffNetObserver()`
    ...
    confirm a feature works *when invoked*. Prove it is invoked: count what the store
