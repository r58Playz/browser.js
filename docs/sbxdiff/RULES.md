...
191.  **The same-origin boundary between two guest origins does not exist, and
      ...
      The first attempt hung on the accepted store. Logging every denied member
      named the first blocker: `$scramjet__eval`. The rewriter turns a handful
      of property reads into `$scramjet__<name>` and the client reads `eval` off
      every subframe it hooks, so the gate was denying scramjet the frame rather
      than denying the page another origin. Letting the whole `$scramjet__`
      prefix through fixed that one -- and the store still hung, so there is at
      least one more.

      Do not try a third time by guessing. Route scramjet's own frame reads
      through the native accessor first (`client.native[iface](el).contentWindow`,
      which already exists for exactly this), so that the trap is only ever seen
      by the page; then the gate has nothing legitimate left to break.
