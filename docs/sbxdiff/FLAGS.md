...
| `--sbxdiff-virtual-time-policy=<p>` | `deterministic` (default), `advance`, or `pause`. The default **deadlocks a service-worker sandbox**: it pauses virtual time while a load is outstanding, and that load is served by a worker whose transport needs timers to progress, so neither side ever moves. Use `advance` there; the budget is still the fence. |
