...
| `SBXDIFF_LOGICAL_CLOCK`         | Replaces the wall clock with one that advances only when a GUEST timer fires, by that timer's delay, so `Date.now()` is a function of the page's own timer chain rather than of the machine. Read by `gin::V8Platform::CurrentClockTime*` and by `Event.timeStamp`. Needs `--sbxdiff-shim-scripts` in the sandbox to know whose timer is whose; without it every timer under a proxy looks like the shim's and the clock never moves at all (RULES.md #99, #101). |
| `SBXDIFF_OBSERVABLE_STEP_US`    | Answers `performance.now()` from a per-realm READ COUNTER stepping `n` µs, rather than from a clock. Cloudflare's challenge measures the machine with `while (performance.now() - start < 100) digest()` and reports the iteration count; a counter makes that loop run the same number of times wherever it runs (5000 against 5000, from 5700 against 6496). 0 or unset is off.                                                                                 |
| `SBXDIFF_SUBFRAME_VT_BUDGET_MS` | Virtual-time budget for a renderer whose main frame is remote. Only matters with virtual time on, which the rym recipe no longer uses.                                                                                                                                                                                                                                                                                                                            |
| `SBXDIFF_PIN_SHIM_COST`         | Pins `performance.memory` and `decodedBodySize` so the shim's own footprint does not show through. Off by default: measured on rym it moved the bucket count 7 → 4 and left the request bodies at 7 → 7, so it buys less than it costs in fidelity.                                                                                                                                                                                                               |
| `SBXDIFF_LOG_TIMER_ATTR`        | One `sbxdiff: timer depth=D ms=N at=T` line per `setTimeout`, where D is how far down the stack the nearest guest frame is and -1 means none. The two sides' logs diff directly, which is what answers "which timers is the sandbox counting that the oracle is not" rather than restating that its clock runs ahead.                                                                                                                                             |
